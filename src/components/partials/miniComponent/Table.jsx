...
import { CardHeader, CategoryPhotoModal, CategoryDetailsModal, TableSkeleton } from "@/components";
...
const getCategoryList = async (pageNumber = 1) => {
  try {
    setLoading(true);
    const response = await axiosInstance.get(`${Constants.BASE_URL}/${props.api_end_point_url}?page=${pageNumber}&search=${input.search}&order_by=${input.order_by}&direction=${input.direction}&per_page=${input.per_page}`);      
